...
Eigen::Isometry3d ReadCalibration(const std::string& calibration_file)
{
  ...
  while (std::getline(input, line))
  {
    ...
    ss >> header >> rot(0, 0) >> rot(0, 1) >> rot(0, 2) >> pos(0) >> rot(1, 0) >> rot(1, 1) >> rot(1, 2) >> pos(1) >>
...
void ReadPointcloud(const std::string& cloud_file, const Eigen::Isometry3d& transform, PointCloudT& points)
{
...
int main(int argc, char** argv)
{
  ...
  if (options_res.count("pc") || velodyne_path.empty() || poses_file.empty() || calibration_file.empty())
  {
  ...
  Bonxai::ProbabilisticMapT<Bonxai::Empty> bonxai_map(voxel_size);
  ...
  std::vector<Bonxai::Empty> empty;
  bonxai_map.getOccupiedVoxels(bonxai_result, empty);
