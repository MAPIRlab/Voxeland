...
public:
  ...
  struct CellT
  {
    ...
    CellT(uint8_t _r, uint8_t _g, uint8_t _b)
      : update_id(0)
      , probability_log(UnknownProbability)
      , r(_r)
      , g(_g)
  ...
  ProbabilisticMapRGB(double resolution);
  ...
  void addHitPoint(const Point3D& point);

  // This function is usually called by insertPointCloud
  // We expose it here to add more control to the user.
  // Once finished adding points, you must call updateFreeCells()
  void addMissPoint(const Point3D& point);
  ...
  void getOccupiedVoxels(std::vector<Bonxai::CoordT>& coords, std::vector<Bonxai::ColorT>& colors);
  ...
  void getOccupiedVoxels(std::vector<PointT>& points)
  {
    ...
    thread_local std::vector<Bonxai::ColorT> colors;
    coords.clear();
    colors.clear();
    getOccupiedVoxels(coords, colors);
    for (int i=0; i < coords.size(); i++)
    {
      const auto p = _grid.coordToPos(coords[i]);
      points.emplace_back(p.x, p.y, p.z, colors[i].r, colors[i].g, colors[i].b);
    }
  }
...
inline void ProbabilisticMapRGB::insertPointCloud(const std::vector<PointT, Alloc>& points,
                                               const PointT& origin,
                                               double max_range)
{
  ...
  for (const auto& point : points)
  {
    ...
    if (squared_norm >= max_range_sqr)
    {
      ...
      addMissPoint(Point3D(new_point.x(), new_point.y(), new_point.z(), point.r, point.g, point.b));
    }
    else {
      addHitPoint(Point3D(to.x(), to.y(), to.z(), point.r, point.g, point.b));
    }
  }
