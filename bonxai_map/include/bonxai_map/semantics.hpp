...
class SemanticMap {
    ...
    public:
        ...
        std::vector<SemanticObject> convertROSMessageToSemanticMap(const std::vector<vision_msgs::msg::Detection2D>& instances) {
            ...
            std::vector<SemanticObject> localSemanticMap(instances.size(), SemanticObject(default_categories.size()));
        ...
        void integrateNewSemantics(const std::vector<SemanticObject>& localMap) {
            ...
            for(size_t localInstanceID = 0; localInstanceID < localMap.size(); localInstanceID++) {
                ...
                bool fused = false;
                for(size_t globalInstanceID = 0; globalInstanceID < globalSemanticMap.size(); globalInstanceID++) {
                    ...
                    uint8_t globalIdx = std::distance(globalInstance.probabilities.begin(), itGlobal);
        ...
        0xe6194B, 0x3cb44b, 0xffe119, 0x4363d8, 0xf58231,
         0x911eb4, 0x42d4f4, 0xf032e6, 0xbfef45, 0xfabed4,
          0x469990, 0xdcbeff, 0x9A6324, 0xfffac8, 0x800000,
           0xaaffc3, 0x808000, 0xffd8b1, 0x000075
    };
