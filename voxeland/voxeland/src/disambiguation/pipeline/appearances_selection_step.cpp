...
AppeareancesSelectionStep::AppeareancesSelectionStep(
    ...
    uint32_t n_images_per_category, uint32_t n_categories_per_instance) {
  this->appearances_classifier = std::move(classifier);
  this->n_images_per_category = n_images_per_category;
  this->n_categories_per_instance = n_categories_per_instance;
}

void AppeareancesSelectionStep::execute() {
  VXL_INFO(
      "[APPEARANCES_SELECTION] Executing appearances selection step using {} "
      "classifier ...",
      appearances_classifier->get_name());

  std::vector<UncertainInstance> uncertain_instances =
      *context->get_uncertain_instances();
...
void AppeareancesSelectionStep::select_category_appearances(
  ...
  for (auto &uncertain_instance : uncertain_instances) {
    ...
    auto categories =
        choose_selected_categories(uncertain_instance.get_instance()->results);
 ...
 *
 * @param sorted_results map of category names and probabilities sorted in
 * descending order.
 */
std::vector<std::string> AppeareancesSelectionStep::choose_selected_categories(
  ...
  std::vector<std::pair<std::string, double>> sorted_results =
      sort_results_map(results);
  std::vector<std::string> selected_categories;

  // Iterate n_categories times to choose the categories with the highest
  // probabilities
  int count = 0;
  for (auto it = sorted_results.begin();
       it != sorted_results.end() && count < n_categories_per_instance;
       ++it, ++count) {
...
AppeareancesSelectionStep::sort_results_map(
    std::map<std::string, double> &results) {
