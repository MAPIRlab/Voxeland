...
namespace voxeland_server
{
    ...
    void VoxelandServer::SelectObjectsAndDraw()
    {
        ...
        for (size_t i = 0; i < semantics.globalSemanticMap.size(); i++)
        {
            if (semantics.globalSemanticMap.at(i).isStillValid())
            {
                ...
                ImGui::SameLine();
                // Get category name from CategoryManager
                CategoryManager& catManager = CategoryManager::getInstance();
                uint32_t catIdx = semantics.globalSemanticMap.at(i).mostLikelyCategory();
                std::string catName = catManager.getCategoryName(catIdx);
                ImGui::Text("%s", fmt::format("- {}", catName).c_str());
            }
            else
    ...
    void VoxelandServer::PrintVoxelInfo(const Bonxai::Point3D& point)
    {
        ...
        ImGui::Text("Voxel (%d, %d, %d):\n%s", coord.x, coord.y, coord.z, voxeland::GetVoxelDescription(cell->data).c_str());
    }
    ...
    void VoxelandServer::PrintInstanceInfo()
    {
        ...
        if (itemSelectedIdx >= (int)semantics.globalSemanticMap.size())
            itemSelectedIdx = 0;

        if (ImGui::BeginCombo("Selected Instance", semantics.globalSemanticMap.at(itemSelectedIdx).instanceID.c_str()))
        {
            for (size_t i = 0; i < semantics.globalSemanticMap.size(); i++)
            {
                if (!semantics.globalSemanticMap.at(i).isStillValid())
                    continue;
                if (ImGui::Selectable(semantics.globalSemanticMap.at(i).instanceID.c_str()))
                    itemSelectedIdx = i;
            }
            ImGui::EndCombo();
        }
        ...
        CategoryManager& catManager = CategoryManager::getInstance();
        for (const auto& [catIdx, alpha] : semantics.globalSemanticMap.at(itemSelectedIdx).alphaParamsCategories)
        {
            if (alpha > 0)
            {
                std::string catName = catManager.getCategoryName(catIdx);
                ImGui::Text("%s: %f", catName.c_str(), alpha);
            }
        }
